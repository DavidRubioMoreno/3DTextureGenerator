...
constexpr int NOISE_FRACTAL_OCTAVES = 4;
...
// Función para rellenar slices desde un buffer 3D ya generado
void FillSlicesFromBuffer(
    std::vector<uint8_t>& image,
    anl::CArray3Dd& buffer,
    int zStart,
    int zEnd
)
{
...
int main()
{
    ...
    if (SEAMLESS_MODE != SeamlessMode::None) {
        std::cout << "Using seamless mode. Generating 3D volume in main thread..." << std::endl;

        // Generar el volumen 3D una sola vez en el hilo principal
        ...
        std::cout << "Volume generated. Distributing slices to threads..." << std::endl;

        // Ahora distribuir el llenado del atlas entre hilos
        ...
        for (int t = 0; t < NUM_THREADS; t++)
        {
            ...
            threads.emplace_back(FillSlicesFromBuffer, std::ref(image), std::ref(buffer), zStart, zEnd);
        }
        ...
        std::cout << "Using standard mode with " << NUM_THREADS << " threads..." << std::endl;
